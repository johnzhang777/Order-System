...
import com.seckill.ordersystem.redis.RedisKeyConstant;
...
public class SeckillServiceImpl implements SeckillService {
    ...
    public CommonResult<OrderResponseDTO> seckill(Long activityId, Long userId, Integer quantity) {
        ...
        // ✅ 优化：从 Redis 获取商品信息，避免查 DB
        String activityKey = String.format(RedisKeyConstant.SECKILL_ACTIVITY, activityId);
        String activityJson = stringRedisTemplate.opsForValue().get(activityKey);

        SeckillActivity seckillActivity;
        if (activityJson == null) {
            // 缓存未命中，查 DB 并回写
            seckillActivity = seckillActivityMapper.selectById(activityId);
            if (seckillActivity == null) {
                return CommonResult.fail(SeckillErrorCode.ACTIVITY_NOT_FOUND);
            }
            // 回写缓存，过期时间设置为活动结束时间
            stringRedisTemplate.opsForValue().set(activityKey, JSON.toJSONString(seckillActivity), Duration.ofHours(24));
        } else {
